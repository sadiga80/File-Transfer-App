...
//                    sendAcknowledgementBroadcast(AP_ENABLED_ACKNOWLEDGEMENT);
     ...
     * @param addStopAction if true STOP action is added to stop {@link TRANSFItService}
     * @return Notification
     */
    private Notification getShareThemNotification(String text, boolean addStopAction) {
        Intent notificationIntent = new Intent(getApplicationContext(),
                TRANSFItActivity.class);
    ...
    /**
     * Starts {@link TRANSFItServer} and then enables Hotspot with assigned port being part of encoded SSID.
     *
    ...
    private void startFileHostingServer(String[] filePathsTobeServed, int port, final String sender_name) {
        m_fileServer = new TRANSFItServer(getApplicationContext()
    ...
    private static class HotspotChecker extends Handler {
        WeakReference<TRANSFItService> service;

        HotspotChecker(TRANSFItService senderService) {
            this.service = new WeakReference<>(senderService);
        }

        @Override
        public void handleMessage(Message msg) {
            TRANSFItService senderService = service.get();
