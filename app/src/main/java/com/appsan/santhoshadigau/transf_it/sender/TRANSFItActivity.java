...
import com.appsan.santhoshadigau.transf_it.utils.DividerItemDecoration;
import com.appsan.santhoshadigau.transf_it.utils.RecyclerViewArrayAdapter;
import com.appsan.santhoshadigau.transf_it.utils.Utils;
import com.appsan.santhoshadigau.transf_it.utils.HotspotControl;
import com.appsan.santhoshadigau.transf_it.utils.WifiUtils;
...
import static com.appsan.santhoshadigau.transf_it.utils.Utils.getRandomColor;


public class TRANSFItActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (null != getIntent() && getIntent().hasExtra(TRANSFItService.EXTRA_FILE_PATHS))
            m_sharedFilePaths = getIntent().getStringArrayExtra(TRANSFItService.EXTRA_FILE_PATHS);
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    ...
                    if (Build.VERSION.SDK_INT >= 23 &&
                            ...
                            Utils.getTargetSDKVersion(getApplicationContext()) >= 23 && !Settings.System.canWrite(TRANSFItActivity.this)) {
        ...
        m_p2pServerUpdatesListener = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (isFinishing() || null == intent)
                    return;
                int intentType = intent.getIntExtra(TRANSFItService.ShareIntents.TYPE, 0);
                if (intentType == TRANSFItService.ShareIntents.Types.FILE_TRANSFER_STATUS) {
                    String fileName = intent.getStringExtra(TRANSFItService.ShareIntents.SHARE_SERVER_UPDATE_FILE_NAME);
                    updateReceiverListItem(intent.getStringExtra(TRANSFItService.ShareIntents.SHARE_CLIENT_IP), intent.getIntExtra(TRANSFItService.ShareIntents.SHARE_TRANSFER_PROGRESS, -1), intent.getStringExtra(TRANSFItService.ShareIntents.SHARE_SERVER_UPDATE_TEXT), fileName);
                } else if (intentType == TRANSFItService.ShareIntents.Types.AP_DISABLED_ACKNOWLEDGEMENT) {
                    shouldAutoConnect = false;
                    resetSenderUi(false);
                }
            }
        };
        registerReceiver(m_p2pServerUpdatesListener, new IntentFilter(TRANSFItService.ShareIntents.SHARE_SERVER_UPDATES_INTENT_ACTION));
    }
    ...
    private void disableAp() {
        ...
        Intent p2pServiceIntent = new Intent(getApplicationContext(), TRANSFItService.class);
        p2pServiceIntent.setAction(TRANSFItService.WIFI_AP_ACTION_STOP);
        startService(p2pServiceIntent);
        isApEnabled = false;
    }

    /**
     * Starts {@link TRANSFItService} with intent action {@link TRANSFItService#WIFI_AP_ACTION_START} to enableShareThemHotspot Hotspot and start {@link TRANSFItServer}.
     */
    private void startP2pSenderWatchService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), TRANSFItService.class);
        p2pServiceIntent.putExtra(TRANSFItService.EXTRA_FILE_PATHS, m_sharedFilePaths);
        if (null != getIntent()) {
            p2pServiceIntent.putExtra(TRANSFItService.EXTRA_PORT, getIntent().getIntExtra(TRANSFItService.EXTRA_PORT, 0));
            p2pServiceIntent.putExtra(TRANSFItService.EXTRA_SENDER_NAME, getIntent().getStringExtra(TRANSFItService.EXTRA_SENDER_NAME));
        }
        p2pServiceIntent.setAction(TRANSFItService.WIFI_AP_ACTION_START);
        startService(p2pServiceIntent);
    }

    /**
     * Starts {@link TRANSFItService} with intent action {@link TRANSFItService#WIFI_AP_ACTION_START_CHECK} to make {@link TRANSFItService} constantly check for Hotspot status. (Sometimes Hotspot tend to stop if stayed idle for long enough. So this check makes sure {@link TRANSFItService} is only alive if Hostspot is enaled.)
     */
    private void startHostspotCheckOnService() {
        Intent p2pServiceIntent = new Intent(getApplicationContext(), TRANSFItService.class);
        p2pServiceIntent.setAction(TRANSFItService.WIFI_AP_ACTION_START_CHECK);
        startService(p2pServiceIntent);
    }

    /**
     * Calls methods - {@link TRANSFItActivity#updateApStatus()} & {@link TRANSFItActivity#listApClients()} which are responsible for displaying Hotpot information and Listing connected clients to the same
     */
     ...
     * Constantly adds/updates receiver items on {@link TRANSFItActivity#m_receiversList}
    ...
    /**
     * Adds a {@link HotspotControl.WifiScanResult} item to {@link TRANSFItActivity#m_receiversListAdapter} if not already added
     *
...
//            m_receiversListAdapter.remove(new WifiApControl.Client(ip, null, null));
    ...
    static class ShareUIHandler extends Handler {
        WeakReference<TRANSFItActivity> mActivity;

        static final int LIST_API_CLIENTS = 100;
        static final int UPDATE_AP_STATUS = 101;

        ShareUIHandler(TRANSFItActivity activity) {
        ...
        public void handleMessage(Message msg) {
            ...
            TRANSFItActivity activity = mActivity.get();
